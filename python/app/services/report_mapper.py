...
# Misma lista fija que MO_Content en el frontend
...
MATERIAL_TIPOS_VISIBLE = {"PROTECCIÓN", "CABLE"}
...
def _map_equipos(items: list[EquipoItem]) -> list[PdfLineItem]:
    ...
    for item in items:
        ...
        lines.append(
            PdfLineItem(
                index=0,
                codigo=_to_str(info.cod_producto),
...
def map_report_form(payload: ReportFormPayload) -> ReportPdfData:
    ...
    if cotizacion:
        ...
        tasa = _to_float(cotizacion.tasa_cambio)
        if tasa > 0:
            tasa_cambio = tasa
    ...
    # Totales en soles cuando hay tipo de cambio; si no, se trabaja en USD
    use_soles = tasa_cambio > 1
