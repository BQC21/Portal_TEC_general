...
export function productsToRows(products: Product[]) {
    ...
    // Para xlsx: cada fila es un array de valores en el mismo orden que headers

    return { csvRows, headers };
}
 ...
 */
export async function prepareAndDownloadFile(products: Product[], fmt: string, nameNoExt: string, includeHeadersFlag: boolean) {
    ...
    if (normalizedFmt === "csv") {
        const csvContent = generateCSV(products, includeHeadersFlag);
        // CSV mime type
        triggerDownload(csvContent, "text/csv;charset=utf-8;", finalName);
