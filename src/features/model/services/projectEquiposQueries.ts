import { mapProject_EquiposToSupabaseRow, mapSupabaseRowToProject_Equipos } from "@/features/model/mapping/project_equipos_mapping";
...
export async function getJoinProjectEquipos(): Promise<Project_Equipos[]>{
    ...
    const { data, error } = await supabase
        .from(PROJECTS_EQUIPOS_TABLE)
        .select("*,equipo_principales(*),proyectos(*)")
