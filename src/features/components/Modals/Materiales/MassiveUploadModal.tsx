...
function mapRowToMaterial(row: string[], offset = 0): MaterialUploadRow {
	return {
		...
		parte_electrica: readSpreadsheetText(row, offset + 6),
		unidad: readSpreadsheetText(row, offset + 7),
		precio_soles: readSpreadsheetNumber(row, offset + 8),
		precio_dolares: readSpreadsheetNumber(row, offset + 9),
		igv: normalizeSpreadsheetPercent(readSpreadsheetNumber(row, offset + 10)),
		precio_soles_igv: readSpreadsheetNumber(row, offset + 11),
		precio_dolares_igv: readSpreadsheetNumber(row, offset + 12),
	};
}
...
export function MassiveUploadModal({ onClose, onSuccess }: MassiveUploadModalProps) {
	...
	async function handleSubmit(event: FormEvent<HTMLFormElement>) {
		...
		try {
			...
			setDetectedHeaderRow(matchedSheet.headerRow ?? null);

			if (matchedSheet.rows.length === 0) {
				throw new Error("El archivo no contiene filas para importar.");
			}

			const payload = matchedSheet.rows.map((row) => mapRowToMaterial(row, matchedSheet.headerStart));
			...
			const numericKeys: Array<keyof MaterialUploadRow> = [
				"precio_soles",
				"precio_dolares",
				"igv",
				"precio_soles_igv",
			...
			const dbPayload = payload.map((r, rowIdx) => {
				...
				return copy as unknown as MaterialUploadRow;
			});
			...
			const offending: { row: number; field: keyof MaterialUploadRow; value: number | string }[] = [];

			dbPayload.forEach((r, idx) => {
				const checks: Array<keyof MaterialUploadRow> = [
					"precio_soles",
					"precio_dolares",
					"igv",
					"precio_soles_igv",
			...
			const { error: insertError } = await supabase.from(MATERIALES_TABLE).insert(payload);
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4">
			<div className="max-h-[95vh] w-full max-w-2xl overflow-hidden rounded-3xl bg-white shadow-2xl">
				<div className="flex items-center justify-between border-b border-slate-200 px-6 py-5">
					<div>
						<h2 className="text-2xl font-bold text-slate-900">Subida masiva de materiales</h2>
						<p className="text-sm text-slate-600">Selecciona un archivo XLSX con la estructura de la hoja de materiales eléctricos.</p>
					</div>
					<button
						type="button"
						onClick={onClose}
