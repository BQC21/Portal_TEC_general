...
export function MassiveCleanModal({
	...
	async function handleClean() {
		...
		try {
			for (const relatedTableName of relatedTableNames) {
				await bulkDeleteAllRows(relatedTableName);
			}
			await bulkDeleteAllRows(tableName);
			onSuccess?.();
			onClose();
		} catch (cleanError) {
			const message =
				cleanError instanceof Error ? cleanError.message : "No se pudo completar la limpieza masiva.";
			setError(message);
		} finally {
			setIsCleaning(false);
		}
					...
					<div className="w-full max-w-md rounded-3xl bg-white shadow-2xl">
						...
						<div className="space-y-5 px-6 py-6">
							<p className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-900">
								Se eliminarán {totalRows} registros de {entityLabel}.
							</p>
