...
import { DeleteSupplierModalProps } from "@/lib/types/components/General/modals";

export function DeleteSupplierModal({ supplier, onDeleteSupplier, onClose }: DeleteSupplierModalProps) {

    // Aceptar actualizacion
    async function handleDeleteZone(event: React.FormEvent<HTMLFormElement>) {
        event.preventDefault();
        await onDeleteSupplier(String(supplier.id));
        onClose();
    }
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4">
            ...
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-5">
                <h2 className="text-2xl font-bold text-slate-900">Eliminar Proveedor</h2>
                <button
                    type="button"
                    onClick={onClose}
            ...
            <form onSubmit={handleDeleteZone} className="max-h-[calc(95vh-88px)] overflow-y-auto px-6 py-6">
                <AddProductReadonlyField
                    ...
                    value={String(supplier.nombre)} 
                />
                <AddProductReadonlyField
                    label="Categoría del proveedor"
                    value={String(supplier.categoria)} 
                />
